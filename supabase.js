...
// Note: passwords are SHA-256 hashed before storage (see supabase.js crypto section).
// Supabase anon key is a public key by design (safe to store client-side per Supabase docs).
const ls = (k, d = null) => { try { const v = localStorage.getItem(k); return v !== null ? JSON.parse(v) : d; } catch { return d; } };
// lsw never stores raw passwords — callers hash first via sha256()
